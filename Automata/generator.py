...
class Tran:
    def __init__(self, tran_id, source, label, intervals, isReset, target):
    ...
    def show(self):
        return {self.tran_id: [self.source, self.label, self.intervals, self.isReset, self.target]}
    ...
    def random_trans_source(self, tran_id, source, sigmasize, partitionsize):
        ...
        for label in labels:
            ...
            for i in range(next_trans_num):
                isReset = random.choice(['r', 'n'])
                temp_tran = Tran(str(tid), source, label, intervals[i], isReset, target[i])
...
def buildjson(g, filename):
    ...
    gdict = {"states": g.s, "inputs": g.sigma, "trans": tran_dict, "initState": g.init, "acceptStates": g.accept}
...
def main():
    filename = "5_3_2"
    n, k, m = filename.split('_')  # statesNumber, sigmaSize, partitionSize
    g = OTAGenerator(filename, int(n), int(k), int(m))
    while not validation(g):
        g = OTAGenerator(filename, int(n), int(k), int(m))
