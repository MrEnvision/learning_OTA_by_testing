...
def hpyCompare(stableHpy, hypothesisOTA, upperGuard, targetSys, targetFullSys, mqNum, metric):
    ...
    compareTrace = getMinCtxList(complementOTA, upperGuard)  # 返回LTWs
    ...
    for state in targetSys.states:
        ...
        for key, value in guardDict.items():
            ...
            if len(addGuards) > 0:
                sinkState = 'sink'
                sinkFlag = True
                tempTran = OTATran(tranNumber, state, key, addGuards, True, sinkState)
...
# Step4 计算最短反例
def getMinCtxList(complementOTA, upperGuard):
    ...
    if len(shortestPath) == 0:
        ...
        # 计算最短反例
        compareTrace = []
        trace, nowTime = getStateTrace(complementOTA, shortestPath)
        compareTrace += trace
