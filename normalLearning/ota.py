...
import json
from normalLearning.interval import Constraint, complement_intervals
...
def buildOTA(jsonfile, otaflag):
    ...
    with open(jsonfile, 'r') as f:
        data = json.load(f)
        name = 'targetSys'
        locations_list = [l for l in data["states"]]
        sigma = [s for s in data["inputs"]]
        trans_set = data["trans"]
        initstate = data["initState"]
        accept_list = [l for l in data["acceptStates"]]
